...
import { FiArrowRight, FiKey, FiHash, FiChevronDown, FiChevronUp, FiLogOut } from 'react-icons/fi';
import toast from 'react-hot-toast';

export default function ConnectionBox({isJoining, setIsJoining}: {isJoining: boolean, setIsJoining: (value: boolean) => void}) {
  const { isMetamaskConnected, isMetaMaskInstalled, metamaskConnectedWallet, connectToMetamask } = useWallet();
    ...
    <div className="bg-white/80 dark:bg-[#161b22]/80 rounded-[32px] p-8 shadow-2xl border border-gray-100 dark:border-white/5 backdrop-blur-xl">
        ...
        <div className="space-y-4">
            ...
            <>
                {!isMetamaskConnected ? (
                    ...
                    <div className="space-y-3">
                        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-500/30 rounded-2xl flex items-center justify-between">
                             <div className="flex items-center gap-3">
                                ...
                                <div className="text-left">
                                    <p className="text-[10px] uppercase font-bold text-gray-500 dark:text-gray-400 tracking-wider">Connected Wallet</p>
                        ...
                        >
                            <span>{isLoading ? 'Verifying...' : 'Connect with Wallet'}</span>
          ...
          <p className="pt-2 text-center text-sm text-gray-500">
             New here? <button onClick={() => setIsJoining(true)} className="text-blue-600 dark:text-blue-400 font-bold hover:underline">Join here</button>
          </p>
        </div>
    </div>
  );
