...
import { FiCopy, FiLogOut, FiUser, FiKey, FiGift, FiExternalLink, FiShield, FiCheckCircle } from 'react-icons/fi';
...
export default function SettingsPage() {
  ...
  const [invitationCodes, setInvitationCodes] = useState<any[]>(user?.invitationCodes || []);
      ...
      <div className="flex flex-col items-center justify-center min-h-[70vh] px-4">
        ...
        >
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-8">
            Please connect your decentralized identity to manage your protocol settings.
          </p>
          <button
  ...
  const handleCreateInvitationCode = async () => {
    try {
      if (!isValidAddress(recipientAddress)) return toast.error('Invalid Ethereum address');
      if (!expireAt) return toast.error('Select an expiration date');
      
      const expiresMs = new Date(expireAt).getTime();
      if (expiresMs <= Date.now()) return toast.error('Expiration must be in the future');

      setIsCreatingCode(true);
      const result: any = await service.createInvitationCode({
        status: 'silver', 
        invitedTo: recipientAddress.trim(), 
        expireAt: expiresMs
      });
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      <header className="mb-10">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2">Manage your protocol identity and access controls.</p>
      </header>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          >
            <div className="flex flex-col items-center text-center">
              ...
              <h2 className="text-2xl font-bold dark:text-white mb-1">{user?.username || 'Protocol User'}</h2>
              ...
              <div className="grid grid-cols-2 gap-4 w-full mb-8">
                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5">
                  ...
                  <p className="text-sm font-bold text-blue-600">{user?.status || 'Active'}</p>
                </div>
                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5">
                  <p className="text-[10px] uppercase font-bold text-gray-400 mb-1">Tier</p>
          ...
          >
            <div className="p-8 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/[0.02]">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h2 className="text-xl font-bold dark:text-white">Device Synchronization</h2>
              </div>
              <p className="text-sm text-gray-500">Scan this code to instantly authorize a secondary hardware device.</p>
            </div>
            ...
            <div className="p-8 flex flex-col md:flex-row gap-8 items-center">
              ...
              <div className="flex-1 w-full space-y-4">
                <div className="p-4 bg-gray-50 dark:bg-black/20 rounded-2xl border border-gray-200 dark:border-white/5">
                  <p className="text-[10px] font-bold text-gray-400 uppercase mb-2">Auth URL</p>
          ...
          >
            <div className="p-8 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/[0.02]">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h2 className="text-xl font-bold dark:text-white">Invitations</h2>
              </div>
              <p className="text-sm text-gray-500">Onboard verified partners to the protocol ecosystem.</p>
            </div>

            <div className="p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Recipient Address</label>
                  <input
                    type="text"
                ...
                >
                  {isCreatingCode ? <span className="animate-pulse">Generating Secure Code...</span> : 'Generate Invitation'}
                </button>
              </div>

              {/* List */}
              <div className="space-y-3">
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Active Invitations</h3>
                <AnimatePresence>
                  {invitationCodes.length > 0 ? (
                    invitationCodes.map((code: string, idx: number) => (
                      <motion.div 
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        key={idx} 
                        className="group flex items-center justify-between p-4 bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/5 hover:border-blue-500/30 transition-all"
                      >
                        <div className="flex flex-col">
                          <span className="text-sm font-mono font-bold dark:text-gray-200">
                            {code.slice(0, 12)}...{code.slice(-8)}
                          </span>
                          <span className="text-[10px] text-gray-400 mt-1">Status: <span className="text-green-500 font-bold">ACTIVE</span></span>
                        </div>
                        <button
                          onClick={() => handleCopy(code, 'Code')}
                          className="p-3 bg-white dark:bg-white/10 rounded-xl shadow-sm opacity-0 group-hover:opacity-100 transition-all hover:text-blue-500"
                        >
                    ...
                    <div className="text-center py-10 bg-gray-50/50 dark:bg-white/5 rounded-3xl border border-dashed border-gray-200 dark:border-white/10">
                      <p className="text-sm text-gray-400">No invitation codes found.</p>
                    </div>
                  )}
                </AnimatePresence>
              </div>
